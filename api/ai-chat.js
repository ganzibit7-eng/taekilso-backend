...
- 색상은 전통 오행-색 대응의 상징적 생활 활용으로만 설명하세요. 용신/희신 계산값이 없다면 '절대적인 행운색'이나 '반드시 피해야 할 색'으로 단정하지 마세요.
- 숫자는 개인의 고유 행운번호·로또번호·당첨번호를 객관적으로 확정할 수 있다고 말하지 마세요. 전통 오행-숫자 대응을 소개할 때는 참고 체계임을 밝히세요.
...
- '나한테 좋은 거 전부 알려줘'라고 하면 ①핵심 기질 ②강점 ③보완점 ④일/직업 방식 ⑤재물 관리 ⑥관계 방식 ⑦참고할 색·환경·활동 ⑧숫자·방향의 전통적 대응과 한계 순으로, 확인 가능한 항목만 정리하세요.
...
- 재물에서는 충동 소비, 공동 금전거래, 과도한 레버리지, 단기 투기, 지인과의 돈거래 등 현실적 위험을 사주 근거가 있을 때만 일반적 관리 조언으로 설명하세요. 특정 투자상품 매수·매도 지시는 하지 마세요.
...
- 사용자가 '조심해야 할 거 전부 알려줘'라고 하면 ①성격/판단 ②재물 ③직업/일 ④연애/대인관계 ⑤생활리듬 ⑥의사결정 습관 순으로, 실제 근거가 있는 항목만 정리하세요.
- 각 주의점은 가능하면 '왜 그렇게 보는지'를 사주 데이터 근거와 함께 짧게 설명하고, 바로 뒤에 '현실적인 대처법'을 붙이세요.
...
async function rollbackUsageReservation(uid, reservation) {
  if (!reservation || !reservation.kind) {
    return;
  }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // body.uid는 호환용 참고값일 뿐입니다.
    // 권한 판단에는 검증된 토큰 UID만 사용합니다.
    ...
    // 사주로 답하면 안 되는 고위험 질문은
    // Anthropic 호출/사용량 차감 전에 바로 제한합니다.
    const restricted =
    ...
    // 기존 프론트는 현재 질문을 history에도 먼저 넣어서
    // 보낼 수 있습니다.
    // 마지막 user 메시지가 현재 질문과 같으면 한 번 제거해
    // Claude에 중복 전달되지 않게 합니다.
...
제공된 데이터만 근거로 사용하고, 제공되지 않은 합충·대운·세운·특정 날짜를 만들어내지 마세요.`;
      ...
      // sajuSummary가 없고 구조화 프로필만 넘어온
      // 예외적인 경우를 위한 보조 정보입니다.
      sajuContext =
        `\n\n[손님의 입력 프로필]\n${
          JSON.stringify(
            sajuProfile
...
현재 날짜는 달력 기준 안내에만 사용하고, 세운·월운 등 명리 계산값을 현재 날짜만으로 만들어내지 마세요.` +
        ...
        await fetch(
          'https://api.anthropic.com/v1/messages',
          {
            ...
            body: JSON.stringify({
              ...
              // 장문 상담도 문장 중간에서 잘리지 않도록
              // 여유를 둡니다.
              // 실제 과금은 생성된 출력 토큰 기준입니다.
              max_tokens: 1600,
    ...
    // 모델이 Markdown 문법을 이스케이프해서 보낸 경우
    // \---, 1\.처럼 노출되지 않도록 안전한 범위에서만 정리합니다.
    replyText =
      replyText
        .replace(
          /\\([#*_~`>\-])/g,
