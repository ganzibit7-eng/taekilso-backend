...
당신의 이름은 '연우'입니다. 택일소의 AI 사주 상담사이며, 손님이 이름을 물으면 "연우예요"라고 자연스럽게 답하세요.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const questionCredits = userData.aiQuestionCredits || 0;
    let usingCredit = false; // 이번 요청이 무료 3회를 다 쓰고 "질문권"에서 차감되는 경우인지
 
 
    // ========================================================
    // 무료 사용자 제한 (무료 3회 → 남은 질문권 → 그래도 없으면 차단)
    // ========================================================
    ...
    if (!isPremiumNow) {
      ...
      if (freeUsed >= FREE_TURN_LIMIT) {
        ...
        if (questionCredits > 0) {
          usingCredit = true; // 질문권이 남아있으면 이걸로 계속 진행합니다.
        } else {
...
`
    ...
    if (!isPremiumNow) {
      ...
      if (usingCredit) {
        // 무료 3회를 이미 다 쓴 상태였고, 질문권으로 진행한 경우: 질문권에서 1개 차감
        await userRef.set(
    ...
    return res.status(200).json({
      ...
      freeUsed:
        isPremiumNow
          ? null
          : (usingCredit ? freeUsed : freeUsed + 1),
      ...
      questionCreditsLeft:
        usingCredit
          ? Math.max(0, questionCredits - 1)
          : (isPremiumNow ? null : questionCredits)
    });
