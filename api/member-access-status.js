...
async function identifiers(uid, decoded) {
  ...
  if (decoded && decoded.email) {
    ids.push(`email:${norm(decoded.email)}`);
  }
  ...
  try {
    ...
    if (user.email) {
      ids.push(`email:${norm(user.email)}`);
    }

    (user.providerData || []).forEach(p => {
      if (p && p.providerId && p.uid) {
        ids.push(`provider:${p.providerId}:${p.uid}`);
      }

      if (p && p.email) {
        ids.push(`email:${norm(p.email)}`);
      }
    });
  } catch (e) {}

  if (String(uid).startsWith('kakao_')) {
    ids.push(`provider:kakao:${String(uid).slice(6)}`);
  }
...
async function getBan(ids) {
  for (const id of ids) {
    ...
    const banDoc = await db
      .collection('memberBans')
      .doc(banId)
      .get();
...
module.exports = async (req, res) => {
  ...
  try {
     ...
     */
    const cooldown =
      await getCooldown(ids);

    if (cooldown) {
      return res.status(403).json({
        error:
          'ACCOUNT_REJOIN_COOLDOWN',

        message:
          '회원탈퇴 후 30일 동안은 동일 계정으로 재가입할 수 없습니다.',

        rejoinAllowedAtMs:
          Number(
            cooldown.rejoinAllowedAtMs ||
            0
          )
      });
    }
